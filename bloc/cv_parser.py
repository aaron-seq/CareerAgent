...
from data.models import CVProfile
from sdk.llm import LocalLLMClient
from helpers.prompts import CV_PARSE_PROMPT
...
class CVParser:
    ...
    def parse_text(self, cv_text: str) -> CVProfile:
        ...
        try:
            logger.info(f"DEBUG: Sending {len(cv_text)} chars to LLM...")
            cv_data = self.llm.generate_json(prompt, temperature=0.2)
            logger.info("DEBUG: LLM response received and parsed.")
            ...
            logger.error(f"ERROR: CV Parsing failed: {e}", exc_info=True)
